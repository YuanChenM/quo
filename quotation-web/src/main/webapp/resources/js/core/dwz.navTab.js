...
var navTab = {
	...
	_contextmenu:function($obj){ // navTab
		var $this = this;
	...
	_loadUrlCallback: function($panel){
		...
		var iff = $('input[type!=hidden]',$panel);
		iff = iff.filter('[type=text]:visible:not([disabled]):not([readonly]):not("#pageinput"):not([tabindex]):first');
		if(iff.length==1){
			setTimeout(function(){try {if($('#skipautofocus',$panel).length==0){
				iff.focus();	
			}} catch (e) {}},500);
		}else{
			setTimeout(function(){try {if($('#skipautofocus',$panel).length==0){
				$panel.find('a:visible:not([disabled]):not("#pageinput"):not([tabindex]):first').focus();
			}} catch (e) {}},500);
		}
	},

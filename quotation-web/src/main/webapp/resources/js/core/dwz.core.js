...
//                + "<div>"+xhr.responseText+"</div>");
	...
	downloadDone:function(message){
		$document = $(document);
		$document.trigger("ajaxStop");
		
		if(parent.alertMsg){
		    parent.alertMsg.error(message);
		}
	},
...
(function($){
	...
	$.fn.extend({
		/**
		 * @param {Object} op: {type:GET/POST, url:ajax, data:ajax, callback: }
		 */
		ajaxUrl: function(op, isNotInitUI){
			var $this = $(this);
			if ($.fn.xheditor) {
			...
			$.ajax({
				...
				success: function(response){
					...
					if (json == null || json.errorCode == null || typeof json == 'xml' || json.errorCode==DWZ.statusCode.ok){
						if (!isNotInitUI){
							$this.html(response).initUI();
						}
						if ($.isFunction(op.callback)) op.callback(response);
					} else {
						if (json.errorCode==DWZ.statusCode.timeout){
							if ($.isFunction(op.error)) op.error();
							...
							if ($.isFunction(op.error)) op.error();
							alertMsg.log(json.message, json.errorCode);
						}
					}
				},
...
//Modify for bug#0085460 at 2013/02/26 by li_hui start
function isNetGreatThanGross(netWeightArgs,grossWeightArgs){
	var grossWeight=grossWeightArgs.val().replaceAll(",","");
	var netWeight=netWeightArgs.val().replaceAll(",","");
	...
	if(parseFloat(grossWeight)<parseFloat(netWeight)){
		grossWeightArgs.removeClass("valid").addClass("error");
		alertMsg.warn(ncompareg,{});
		return false;
	}else{
		grossWeightArgs.removeClass("error").addClass("valid");
		return true;
	}
}
//Modify for bug#0085460 at 2013/02/26 by li_hui end